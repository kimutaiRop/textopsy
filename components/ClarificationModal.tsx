...
export function ClarificationModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 py-8 backdrop-blur-sm">
      <div className="w-full max-w-3xl rounded border border-gray-800 bg-[#0f172a] p-8">
        {/* Header */}
        <div className="mb-8">
          <div className="flex items-start gap-4 mb-4">
            <div className="flex h-12 w-12 items-center justify-center rounded border border-[#b74bff]/30 bg-[#b74bff]/10">
              <IconInfo className="h-6 w-6 text-[#b74bff]" />
            </div>
            <div className="flex-1">
              <p className="text-xs uppercase tracking-[0.3em] text-[#b74bff] font-semibold">Quick check</p>
              <h2 className="mt-2 text-2xl font-bold text-white">We need a little more context</h2>
        ...
        <div className="space-y-5 max-h-[60vh] overflow-y-auto pr-2">
          {questions.map((question, index) => (
            <div 
              key={question.id} 
              className="border border-gray-800 bg-[#1e293b] p-6 rounded"
            >
              <div className="flex items-start gap-4 mb-4">
                <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded border border-[#b74bff]/30 bg-[#b74bff]/10">
                  {areaIcons[question.area] || <IconInfo className="h-5 w-5 text-[#b74bff]" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-3 mb-1">
                    <p className="text-sm font-bold text-white leading-snug">{question.question}</p>
                    <span className="rounded border border-gray-700 bg-gray-800 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-gray-400 whitespace-nowrap">
                      {areaLabels[question.area] || "Context"}
                    </span>
                  </div>
                  {question.helperText && (
                    <p className="text-xs text-gray-500 mt-1.5 leading-relaxed">{question.helperText}</p>
                  )}
                </div>
              </div>
        ...
        <div className="mt-8 flex justify-end gap-3 border-t border-gray-800 pt-6">
          <button
            type="button"
            onClick={onCancel}
            className="rounded border-2 border-gray-800 bg-[#1e293b] px-5 py-3 text-sm font-semibold text-gray-400 transition-colors hover:border-gray-700 hover:text-white hover:bg-gray-800"
          ...
          <button
            ...
            className={`flex items-center gap-2 rounded border-2 px-6 py-3 text-sm font-semibold transition-all ${
